...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        [Header("카메라 타겟")]
        [Tooltip("카메라가 따라갈 타겟 목록입니다.")]
        public List<CameraTarget> CameraTargets = new List<CameraTarget>();

        [Tooltip("활성화하면, 게임 시작 시 카메라를 타겟의 중앙에 즉시 배치합니다.")]
        public bool CenterTargetOnStart;

        [Header("기본 설정")]
        [Tooltip("카메라의 주된 이동 축을 설정합니다. (XY: 2D 사이드 스크롤, XZ: 3D 탑다운)")]
        public MovementAxis Axis;

        [Tooltip("카메라 업데이트 시점을 결정합니다. (LateUpdate: 일반적, FixedUpdate: 물리 기반)")]
        public UpdateType UpdateType;
        
        [Header("추적 설정")]
        [Tooltip("수평(좌우) 방향으로 타겟을 따라갈지 여부입니다.")]
        public bool FollowHorizontal = true;
        [Tooltip("수평 추적의 부드러움 정도입니다. 작을수록 빠르게 따라갑니다.")]
        public float HorizontalFollowSmoothness = 0.15f;

        [Tooltip("수직(상하) 방향으로 타겟을 따라갈지 여부입니다.")]
        public bool FollowVertical = true;
        [Tooltip("수직 추적의 부드러움 정도입니다. 작을수록 빠르게 따라갑니다.")]
        public float VerticalFollowSmoothness = 0.15f;

        [Header("오프셋 설정")]
        [Range(-1f, 1f)]
        [Tooltip("카메라의 수평 위치를 타겟으로부터 얼마나 떨어뜨릴지 결정합니다.")]
        public float OffsetX;

        [Range(-1f, 1f)]
        [Tooltip("카메라의 수직 위치를 타겟으로부터 얼마나 떨어뜨릴지 결정합니다.")]
        public float OffsetY;

        [Tooltip("활성화하면, 오프셋이 화면 크기에 비례하여 적용됩니다. (권장)")]
        public bool IsRelativeOffset = true;

        [Header("기타 설정")]
        [Tooltip("활성화하면, 줌 기능을 카메라의 Z축 이동 대신 FOV(시야각) 변경으로 처리합니다. (원근 카메라 전용)")]
        public bool ZoomWithFOV;

        [Tooltip("활성화하면, Time.timeScale 값에 영향을 받지 않고 카메라가 움직입니다. (예: 게임이 정지해도 카메라는 이동 가능)")]
