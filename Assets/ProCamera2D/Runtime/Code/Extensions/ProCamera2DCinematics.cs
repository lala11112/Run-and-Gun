...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	{
        [Tooltip("시네마틱 시퀀스에서 카메라가 바라볼 타겟 Transform 입니다.")]
		public Transform TargetTransform;

        [Tooltip("카메라가 이 타겟에 도달하는 데 걸리는 시간(초)입니다.")]
		public float EaseInDuration = 1f;
        
        [Tooltip("카메라가 이 타겟에 머무르는 시간(초)입니다. 0 미만으로 설정 시 다음 타겟으로 자동으로 넘어가지 않습니다.")]
		public float HoldDuration = 1f;

        [Tooltip("카메라가 이 타겟을 비출 때의 줌 배율입니다. 1은 줌 없음, 2는 2배 줌인입니다.")]
		public float Zoom = 1f;

        [Tooltip("카메라가 이 타겟으로 이동할 때의 애니메이션 커브입니다.")]
		public EaseType EaseType = EaseType.EaseOut;

        [Tooltip("타겟에 도달했을 때 해당 타겟의 게임오브젝트로 보낼 메시지(메서드) 이름입니다.")]
		public string SendMessageName;

        [Tooltip("SendMessage 호출 시 전달할 문자열 파라미터입니다.")]
	...
	public class ProCamera2DCinematics : BasePC2D, IPositionOverrider, ISizeOverrider
	{
        ...
        [Tooltip("시네마틱 시퀀스를 구성하는 타겟들의 목록입니다.")]
		public List<CinematicTarget> CinematicTargets = new List<CinematicTarget>();

        [Tooltip("시네마틱이 끝난 후, 원래 카메라 위치로 돌아가는 데 걸리는 시간(초)입니다.")]
		public float EndDuration = 1f;

        [Tooltip("시네마틱이 끝날 때의 애니메이션 커브입니다.")]
		public EaseType EndEaseType = EaseType.EaseOut;

        [Tooltip("활성화하면, 시네마틱 카메라의 움직임이 Numeric Boundaries의 영향을 받습니다.")]
		public bool UseNumericBoundaries;

        [Tooltip("활성화하면, 시네마틱 재생 중에 화면 위아래에 검은색 레터박스가 나타납니다.")]
		public bool UseLetterbox = true;

		[Range(0f, .5f)]
        [Tooltip("레터박스의 두께입니다. 0.1은 화면 높이의 10%를 의미합니다.")]
		public float LetterboxAmount = .1f;

        [Tooltip("레터박스가 나타나고 사라지는 데 걸리는 시간(초)입니다.")]
